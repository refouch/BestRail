import pytest
from algo_backend.raptor import RAPTOR   # Adjust path if needed
...
def test_direct_trip(dataset):
    tau = run_raptor(dataset, "A", "D", departure_time=0)
...
def test_impossible_trip(dataset):
    tau = run_raptor(dataset, "B", "G", departure_time=0) # B -> D is impossible 
